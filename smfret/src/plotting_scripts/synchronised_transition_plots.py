...
def filt_df_to_plot2(df, FRET_before, FRET_after, transition_type='low_to_high', min_dwell_before=0):
    """
    Filters for transitions. 
    FRET_before/after can be a float (threshold) or a tuple (min, max range).
    """
    transitions_to_plot = df[df['transition_point'] == True]
    
    # Pre-filter for dwell time
    df_filt = transitions_to_plot[transitions_to_plot['transition_dwell'] > min_dwell_before]

    # Helper function to handle either threshold or range logic
    def apply_bound(series, value, direction):
        if isinstance(value, (tuple, list)):
            # Range logic: value must be between tuple[0] and tuple[1]
            return (series >= value[0]) & (series <= value[1])
        else:
            # Original logic: threshold based on direction
            if direction == 'lower': # Value must be below threshold
                return series < value
            else: # Value must be above threshold
                return series > value

    if transition_type == 'low_to_high':
        # Before must be 'low' (below threshold OR in range)
        # After must be 'high' (above threshold OR in range)
        mask = (apply_bound(df_filt['FRET_before'], FRET_before, 'lower') & 
                apply_bound(df_filt['FRET_after'], FRET_after, 'higher'))
        
    elif transition_type == 'high_to_low':
        # Before must be 'high' (above threshold OR in range)
        # After must be 'low' (below threshold OR in range)
        mask = (apply_bound(df_filt['FRET_before'], FRET_before, 'higher') & 
                apply_bound(df_filt['FRET_after'], FRET_after, 'lower'))
    
    return df_filt[mask].index
...
def master_plot_synchronised_transitions(order, output_folder='Experiment_1-description/python_results', exposure=0.2, frames_to_plot=50, FRET_before=0.3, FRET_after=0.3, datatype='Proportion', filt=True, palette='BuPu', add_time=0, df=None):
    ...
    #cursed specified windows for special filtering
    special_filt_release = filt_df_to_plot2(
        calculated_transitions_df, 
        FRET_before=(0.3, 0.5),  # Range
        FRET_after=0.5,          # Threshold
        transition_type='low_to_high'
        )
    special_filt_binding = filt_df_to_plot2(
        calculated_transitions_df,
        FRET_before=0.5,         # Threshold
        FRET_after=(0.3, 0.5),   # Range
        transition_type='high_to_low'
        )
    ...
    filt_data3, filt_data4 = plot_synchronised_transition2(dfs=calculated_transitions_df, 
                                                          index_to_plot=dnak_stable_release, 
                                                          index_to_plot2=dnak_stable_binding,
                                                          exposure_seconds=exposure, 
                                                          order=order, 
                                                          frame_from_trans=frames_to_plot, 
                                                          save_loc=plot_export, 
                                                          ...
                                                          label='consecutive_transitions', 
                                                          add_time=add_time)
    spec_filt_1, spec_filt_2 = plot_synchronised_transition2(dfs=calculated_transitions_df, 
                                                          index_to_plot=special_filt_release,
                                                          index_to_plot2=special_filt_binding,
                                                          exposure_seconds=exposure, 
                                                          order=order, 
                                                          frame_from_trans=frames_to_plot, 
                                                          save_loc=plot_export, 
                                                          ...
                                                          label='consecutive_transitions', 
                                                          add_time=add_time)
    return percent_trans_meet_criteria_df, calculated_transitions_df, consecutive_from_dnak_release, nonconsecutive_from_dnak_release, filt_data, filt_data2, combined_consec_nonconsec, filt_data3, filt_data4,spec_filt_1, spec_filt_2
